export const createDemoState = () => ({
  version: 1,
  repo: {
    name: 'parcel-ops',
    branch: 'release/checkout-guardrails',
    language: 'JavaScript / TypeScript',
    lastScan: 'fixture snapshot',
  },
  scan: {
    scannedAt: '2026-07-19T06:10:00.000Z',
    files: [
      { file: 'src/api/quote.js', symbols: ['createQuoteResponse'], imports: ['../quote.js'], routes: [] },
      { file: 'src/checkout/CheckoutForm.jsx', symbols: ['CheckoutForm'], imports: ['../quote.js', './submitOrder.js'], routes: [] },
      { file: 'src/checkout/submitOrder.js', symbols: ['submitOrder'], imports: [], routes: [] },
      { file: 'src/quote.js', symbols: ['quoteTotal', 'quoteSummary'], imports: [], routes: [] },
      { file: 'src/receipts/DeliveryNote.jsx', symbols: ['DeliveryNote'], imports: [], routes: [] },
      { file: 'src/tax/adjustments.js', symbols: ['calculateTaxAdjustment'], imports: [], routes: [] },
    ],
    exports: [
      { symbol: 'createQuoteResponse', file: 'src/api/quote.js' },
      { symbol: 'CheckoutForm', file: 'src/checkout/CheckoutForm.jsx' },
      { symbol: 'submitOrder', file: 'src/checkout/submitOrder.js' },
      { symbol: 'quoteTotal', file: 'src/quote.js' },
      { symbol: 'quoteSummary', file: 'src/quote.js' },
      { symbol: 'DeliveryNote', file: 'src/receipts/DeliveryNote.jsx' },
      { symbol: 'calculateTaxAdjustment', file: 'src/tax/adjustments.js' },
    ],
  },
  lanes: [
    {
      id: 'ui-checkout',
      agent: 'Mira / Experience',
      task: 'Prevent accidental double submit on checkout.',
      status: 'airborne',
      files: ['src/checkout/CheckoutForm.jsx'],
      symbols: ['CheckoutForm', 'submitOrder'],
      contracts: ['submit-order'],
      changedFiles: ['src/checkout/CheckoutForm.jsx'],
      evidence: [{ command: 'node --test fixtures/parcel-ops/tests/checkout.test.mjs', result: 'passing', source: 'bundled fixture record', at: '09:42' }],
      note: 'UI-only lane. Do not change pricing behavior.',
      createdAt: '2026-07-17T09:31:00.000Z',
      updatedAt: '2026-07-17T09:42:00.000Z',
    },
    {
      id: 'pricing-rules',
      agent: 'Theo / Pricing',
      task: 'Make quote totals include the delivery threshold.',
      status: 'airborne',
      files: ['src/quote.js', 'src/api/quote.js'],
      symbols: ['quoteTotal', 'quoteSummary'],
      contracts: ['pricing', 'quote-response'],
      changedFiles: ['src/quote.js', 'src/api/quote.js'],
      evidence: [{ command: 'node --test fixtures/parcel-ops/tests/quote.test.mjs', result: 'passing', source: 'bundled fixture record', at: '09:39' }],
      note: 'Owns the quote calculation until handoff.',
      createdAt: '2026-07-17T09:18:00.000Z',
      updatedAt: '2026-07-17T09:39:00.000Z',
    },
    {
      id: 'tax-adjustment',
      agent: 'Sol / Rules',
      task: 'Apply regional tax adjustment to the checkout quote.',
      status: 'holding',
      files: ['src/quote.js', 'src/tax/adjustments.js'],
      symbols: ['quoteTotal', 'calculateTaxAdjustment'],
      contracts: ['pricing', 'tax-adjustment'],
      changedFiles: ['src/tax/adjustments.js'],
      evidence: [{ command: 'node --test fixtures/parcel-ops/tests/tax.test.mjs', result: 'passing', source: 'bundled fixture record', at: '09:41' }],
      note: 'Bundled fixture record only; the real CLI executes proof commands and audits Git before handoff.',
      createdAt: '2026-07-17T09:35:00.000Z',
      updatedAt: '2026-07-17T09:41:00.000Z',
    },
    {
      id: 'receipt-copy',
      agent: 'Ada / Comms',
      task: 'Clarify the delivery threshold in the receipt copy.',
      status: 'queued',
      files: ['src/receipts/DeliveryNote.jsx'],
      symbols: ['DeliveryNote'],
      contracts: ['receipt-copy'],
      evidence: [],
      note: 'Can launch once the copy owner confirms scope.',
      createdAt: '2026-07-17T09:44:00.000Z',
      updatedAt: '2026-07-17T09:44:00.000Z',
    },
  ],
  activity: [
    { time: '09:44', type: 'reserve', lane: 'receipt-copy', text: 'Ada requested a new lane.' },
    { time: '09:41', type: 'evidence', lane: 'tax-adjustment', text: 'Fixture includes a focused tax-adjustment test record.' },
    { time: '09:40', type: 'hold', lane: 'tax-adjustment', text: 'Runway held Sol for a shared pricing symbol.' },
    { time: '09:39', type: 'evidence', lane: 'pricing-rules', text: 'Fixture includes a passing quote-test record.' },
    { time: '09:31', type: 'launch', lane: 'ui-checkout', text: 'Mira was cleared for the checkout lane.' },
  ],
})
